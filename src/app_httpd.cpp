...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  else if (!strcmp(variable, "seglen"))  // jz recording parameters
    avi_length = val;
  else if (!strcmp(variable, "interval"))  //jz
    frame_interval = val;
  else if (!strcmp(variable, "speedup"))  //jz
    speed_up_factor = val;
...
static esp_err_t status_handler(httpd_req_t *req)
{
  ...
  *p++ = '{';
  ...
  p += sprintf(p, "\"seglen\":%d,", avi_length);            //jz recording parameters
  p += sprintf(p, "\"interval\":%d,", frame_interval);      //jz
  p += sprintf(p, "\"speedup\":%d,", speed_up_factor);       //jz
  p += sprintf(p, "\"freespace\":%d", freespace);           //jz
#ifdef CONFIG_LED_ILLUMINATOR_ENABLED
...
static esp_err_t index_handler(httpd_req_t *req)
{
  ...
  if (s != NULL) {
    if (s->id.PID == OV3660_PID) {
      ...
      //index_ov2640
      //return httpd_resp_send(req, (const char *)index_ov5640_html_gz, index_ov5640_html_gz_len);
      int len = sizeof(index_ov2640_html); //45110
      Serial.print("index_ov2640_html len "); Serial.println(len);
      return httpd_resp_send(req, (const char *)index_ov2640_html, len);
    }
  } else {
...
static esp_err_t startrecord_handler(httpd_req_t *req){         //jz
  ...
  if (parse_get(req, &buf) == ESP_OK) {
    uint32_t real = parse_get_var(buf, "now", 0) ;
    //Serial.print("\nreal: "); Serial.println(real);
    struct timeval tv;
    tv.tv_sec = real;
    tv.tv_usec = 0;
...
static esp_err_t stoprecord_handler(httpd_req_t *req){  //jz
